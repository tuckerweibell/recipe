...
<Meta title="Guides/Migration/v15" />
<TableOfContents />

# Migration to Recipe 15
...
1. With the removal of `snitches`, if your app uses server-side rendering (SSR), extracted styles from `stitches` will need to be inserted directly into the html of your app.
...
3. [EzCarousel](?path=/docs/navigation-ezcarousel--docs#ezcarousel) has a new design that now includes page buttons on desktop viewports (`>= 768px`) and horizontal scrolling on mobile viewports (`< 768px`). The following breaking changes have occurred as a result:
