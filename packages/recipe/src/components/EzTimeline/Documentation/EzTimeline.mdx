...
<MigrationAlert component="ez-timeline" />
...
- Be used directly on the an [EzPage](?path=/docs/layout-ezpage--docs), and should never be nested within a card or other styled containers.
...
- An optional `href` link. When provided, the link will highlight the card title as a clickable link, allowing the user to see more content related to the event card. Similar to [EzLink](?path=/docs/navigation-ezlink--docs), when using a client-side routing implementation a `to` path can be used instead of `href`, with the router link component provided via the `as` property.
