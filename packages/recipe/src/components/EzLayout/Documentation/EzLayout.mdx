...
<MigrationAlert component="ez-layout" />
...
- Be used for large-scale page layout. For large-scale layouts, consider using a [Card](?path=/docs/surface-ezcard--docs) within [Page](?path=/docs/layout-ezpage--docs)
...
The `stack` layout arranges content into a vertical column. The `stack` layout can be used, for example, when targeting small screen sizes, to organize content into stacked columns. In many cases, a `stack` layout may not be necessary; block level components will naturally stack in a container, and container components such as [`EzCard`](?path=/docs/surface-ezcard--docs) will apply consistent spacing between content items.
