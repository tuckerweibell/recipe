...
- Be used in place of a more semantic element.
  - For navigation or actions that do not affect the state of the app, use [`EzLink`](?path=/docs/navigation-ezlink--docs). A button does something. A link goes somewhere. Ex. breadcrumbs, redirects, "forgot password", and "learn more" all go somewhere rather than do something, so should be links.
  - For status chips, use [`EzChip`](?path=/docs/data-display-ezchip--docs). Chips are reactive and contextual, whereas buttons are static and predetermined. Ex. a delivery status of "confirmed" is contextual and not a static action, so should be a status chip.
...
<Canvas of={ColorStories.FilledColors} meta={ColorStories} />
<Canvas of={ColorStories.TextColors} meta={ColorStories} />
<Canvas of={ColorStories.InlineColors} meta={ColorStories} />
...
If you want to specify a button size, use the `size` property. We currently support `small`, `medium` (default), and `large`. Button sizes affect the amount of padding around the button content. Applies to all variants except `inline`.

For full width buttons, use the `fullWidth` prop.

<Canvas of={ButtonSizeStories.FilledButtonSizes} meta={ButtonSizeStories} />
<Canvas of={ButtonSizeStories.OutlinedButtonSizes} meta={ButtonSizeStories} />
<Canvas of={ButtonSizeStories.TextButtonSizes} meta={ButtonSizeStories} />
...
<Canvas of={FontSizeStories.FilledFontSizes} meta={FontSizeStories} />
<Canvas of={FontSizeStories.OutlinedFontSizes} meta={FontSizeStories} />
<Canvas of={FontSizeStories.TextFontSizes} meta={FontSizeStories} />
<Canvas of={FontSizeStories.InlineFontSizes} meta={FontSizeStories} />
...
<Unstyled>
  <EzAlert
    use="warning"
    headline="Legacy buttons are deprecated as of v17 and will be removed in v19."
  />
</Unstyled>
