...
- use the `onClick` prop to handle click or touch interactions with the card, typically triggering the navigation action of a link within the card, or to display a [modal dialog](/components/ez-modal)
