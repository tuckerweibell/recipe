...
<MigrationAlert component="ez-rating" />
...
The rating component uses `EzIcon`. For more information about how to provide an icon to Recipe from your application, see [EzIcon](?path=/docs/typography-ezicon--docs).
...
The same values available for the [EzIcon's](?path=/docs/typography-ezicon--docs#icon-color) `color` can be provided as an optional prop.

We currently support theme palette properties (`primary`, `secondary`, `error`, `warning`, `info`, and `success`) as well as all supported [theme colors](?path=/docs/guides-theming--docs#colors) (ex. `common.primary100`).
...
<RelatedComponents components={['EzProgress']} />
