...
<MigrationAlert component="ez-toggle" />
...
- Be used for interactions where the effect of the interaction is immediate. For deferred interactions, such as saving changes when a form is submitted, use [EzCheckbox](?path=/docs/input-ezcheckbox--docs) instead
...
<RelatedComponents components={['EzCheckbox', 'EzField', 'EzFormLayout']} />
