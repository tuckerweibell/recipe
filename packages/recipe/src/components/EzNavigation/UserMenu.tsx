...
type TriggerProps = Omit<HTMLProps<HTMLButtonElement>, 'className'> &
  Pick<MenuProps, 'icon' | 'name'>;

const UserMenuTrigger = forwardRef<HTMLButtonElement, TriggerProps>(
  ({icon, name, ...props}, ref) => {
    return (
      <button
        {...props}
...
const UserMenu: React.FC<MenuProps> = ({icon, name, links, isSidebarOpen, sidebarToggle}) => {
  const ref = useRef();
  ...
  return (
    <>
      ...
      />
      <UserMenuTrigger {...menuTriggerProps} ref={ref} name={name} icon={icon} />
    </>
  );
};
