...
<MigrationAlert component="ez-radio" />
...
- If available options can be collapsed, consider using an `EzField` [select list]('?path=/docs/input-ezfield--docs#select-list') which uses less space.
...
<RelatedComponents components={['EzCheckbox', 'EzField']} />
